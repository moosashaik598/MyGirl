(() => {
  ...
  function setActive(i) {
    ...
    // Pause video when leaving slide 3
  ...
  function revealSlide1() {
    ...
    setTimeout(() => {
      if (index === 0) statsEl.classList.add("is-visible");
    }, 900);

    setTimeout(() => {
      if (index === 0) hintEl.classList.add("is-visible");
    }, 1700);
  }
  ...
  // ===== Slide 3 FX (ribbons + fireworks + balloons) =====
  ...
  function startSlide3FX() {
    ...
    // Gentle balloons behind (re-use .balloon style)
    for (let i = 0; i < 8; i++) {
      const b = document.createElement("i");
      b.className = "balloon";
